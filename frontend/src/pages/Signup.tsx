    ...
    <>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-10 bg-slate-50 overflow-y-auto">
        <div className="w-full max-w-md my-auto">
          ...
          {/* Server Message Area */}
          {serverMsg && (
            <div className={`mb-4 p-3 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-2 duration-300 ${
              serverMsg.type === 'success' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-700 border border-red-100'
            }`}>
              {serverMsg.type === 'success' ? <CheckCircle2 className="w-4 h-4 flex-shrink-0" /> : <AlertCircle className="w-4 h-4 flex-shrink-0" />}
          ...
          <form className="space-y-3" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <div className="relative group">
                  <User className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors ${errors.fullName ? 'text-red-400' : 'text-slate-400 group-focus-within:text-secondary'}`} />
                  <input 
                    {...register('fullName')}
                    type="text" 
                    placeholder="John Doe" 
                    className={`w-full bg-white border-2 rounded-xl py-2.5 pl-10 pr-4 outline-none transition-all font-bold text-dark text-sm shadow-sm ${
                      errors.fullName ? 'border-red-100 focus:border-red-200' : 'border-slate-100 focus:border-secondary/30'
                    }`}
                  />
                </div>
                {errors.fullName && <p className="text-[9px] text-red-500 font-bold ml-1 uppercase tracking-tight">{errors.fullName.message}</p>}
              </div>

              <div className="space-y-1">
                <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">Email Address</label>
                <div className="relative group">
                  <Mail className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors ${errors.email ? 'text-red-400' : 'text-slate-400 group-focus-within:text-secondary'}`} />
                  <input 
                    {...register('email')}
                    type="email" 
                ...
                {errors.email && <p className="text-[9px] text-red-500 font-bold ml-1 uppercase tracking-tight">{errors.email.message}</p>}
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="relative group">
                <Lock className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors ${errors.password ? 'text-red-400' : 'text-slate-400 group-focus-within:text-secondary'}`} />
                <input 
                  {...register('password')}
                  type="password" 
                  placeholder="••••••••" 
              ...
              {errors.password && <p className="text-[9px] text-red-500 font-bold ml-1 uppercase tracking-tight">{errors.password.message}</p>}
            </div>

            {/* Charity Dropdown */}
            <div className="space-y-1">
              <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">Support Charity</label>
              <div className="relative group">
                <Heart className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors ${errors.charityId ? 'text-red-400' : 'text-slate-400 group-focus-within:text-secondary'}`} />
                <select 
                  {...register('charityId')}
                  className={`w-full bg-white border-2 rounded-xl py-2.5 pl-10 pr-4 outline-none transition-all font-bold text-dark text-sm shadow-sm appearance-none cursor-pointer ${
              ...
              {errors.charityId && <p className="text-[9px] text-red-500 font-bold ml-1 uppercase tracking-tight">{errors.charityId.message}</p>}
            </div>
            ...
            <div className="pt-3 border-t border-slate-200">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                  <label className="text-[8px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1">
                    Impact <Heart className="w-2 h-2 text-secondary fill-secondary" />
                  </label>
                  <div className="text-xl font-black text-dark tracking-tighter">
                    {donation}<span className="text-secondary text-xs">%</span>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <div className="text-[8px] font-black uppercase tracking-widest text-slate-400">Level</div>
                  <div className="bg-dark text-white px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest">
              ...
              <div className="relative h-6 flex items-center">
                <input 
                  ...
                  className="absolute w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-secondary z-20"
                  style={{
              ...
              <p className="text-[8px] text-slate-400 font-bold uppercase tracking-widest text-center mt-3 opacity-80">
            ...
            <button 
              disabled={isLoading}
              className={`w-full bg-dark text-white py-3.5 rounded-xl font-black text-base hover:bg-slate-800 transition-all shadow-xl shadow-dark/10 flex items-center justify-center gap-3 group mt-2 ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`}
            >
              {isLoading ? 'Creating Account...' : 'Complete Registration'}
              {!isLoading && <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />}
            </button>

            <p className="text-center text-slate-500 font-bold text-[10px] mt-2">
